...
const SmartContractGenerator: React.FC = () => {
  ...
  const industries = ['Finance', 'Healthcare', 'Real Estate', 'Gaming', 'Supply Chain'];
  ...
  const generateContractCode = async () => {
    if (!industry || !language || !contractDesign || !ercStandard || !userRequirements) {
    ...
    const prompt = `Generate a fully optimized smart contract using ${language} with the following specifications:
    Industry: ${industry}
    Contract Design: ${contractDesign}
    ERC Standard: ${ercStandard}
    Gas Limit: ${gasLimit}
    User Requirements: ${userRequirements}
    Ensure the contract follows best practices and security measures.`;
  ...
  >
    {isCopied ? 'Copied!' : 'Copy Code'}
        ...
        <TextField
          placeholder="Specify your smart contract details and get an optimized code."
          multiline
          rows={2}
          fullWidth
          variant="outlined"
  ...
  >
    {isCopied ? 'Copied!' : 'Copy Code'}
      ...
      <Box display="flex" flexDirection="column" gap={2} width={{ xs: '100%', md: 250 }}>
        <Typography variant="h6" color="white">Generate Smart Contracts</Typography>

        {/** Industry Select */}
        <Select value={industry} onChange={(e) => setIndustry(e.target.value)} displayEmpty sx={selectStyles}>
          <MenuItem value="" disabled>Choose Industry</MenuItem>
          {industries.map((item) => <MenuItem key={item} value={item} sx={menuItemStyles}>{item}</MenuItem>)}
        </Select>
        ...
        <Select value={contractDesign} onChange={(e) => setContractDesign(e.target.value)} displayEmpty sx={selectStyles}>
          <MenuItem value="" disabled>Contract Design</MenuItem>
